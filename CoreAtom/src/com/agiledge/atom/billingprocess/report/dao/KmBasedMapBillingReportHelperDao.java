...
public class KmBasedMapBillingReportHelperDao
 {

	 
		public ArrayList<TripBasedBillingReportHelperDto> getKmBasedMapBillingTripCostReport_monthly(String site, String vendorId, String vehicleType, String fromDate, String toDate, String month , String year, String escort, String constraint  ) {
		...
		//	option=option==null?"":option;
			...
			try {
				...
				String query1 = " select YEAR( td.trip_date) tripYear,  MONTHNAME(td.trip_date) tripMonth, td.vehicleTypeName, sum(targ.mapTripDistance) mapTripDistance, count(*) totTrips, sum(targ.mapTripRate) mapTripRate, sum(targ.mapEscortRate) mapEscortRate   from  td_billing_args targ join " +
						"(select vtp.manualDistance, vtp.escort tEscort, ifnull(vtp.approvalStatus,'Open') approvalStatus, tva.vendorId, td.*, vt.type vehicleTypeName from trip_details td join vehicle_type vt on td.vehicle_type=vt.id join tripvendorassign tva on td.id= tva.tripid  left join vendor_trip_sheet_parent vtp on td.id = vtp.tripId where approvalStatus = '"+SettingsConstant.APPROVAL_TYPE+"' ) td on targ.tripId = td.id join "+
								" (select tbcc.id, tbcc.ac_constraint, tbcc.trip_rate from km_based_trip_config tbc join km_based_map_trip_config  tbcc on tbcc.km_billing_id=tbc.id " + constraintQuery + " ) tbc on tbc.id= targ.mapTripBillingId " +
				...
				System.out.println("mmmm"+query1);
				...
				while (rs.next()) {
					...
					dto.setEscortRate(rs.getDouble("mapEscortRate")); 
				 	dto.setDistanceRange(rs.getString("distanceRange"));
					dto.setTrips(getKmBasedMapBillingTripCostReport_tripWise(site, vendorId, dto.getVehicleType(), fromDate, toDate, dto.getMonthName(), dto.getYear(), escort, constraint));
				...
				System.out.println("Error in getKmBasedMapBillingTripCostReport_monthly "+ e);
			} finally {
		...
		public ArrayList<TripBasedBillingReportHelperDto> getKmBasedMapBillingTripCostReport_daily(String site, String vendorId, String vehicleType, String fromDate, String toDate, String month, String year, String escort, String constraint) {
		...
		//	option=option==null?"":option;
			...
			try {
				...
				String query1 = "  select   td.trip_date, td.vehicleTypeName, sum(targ.mapTripDistance) mapTripDistance, count(*) totTrips, sum(targ.mapTripRate) mapTripRate, sum(targ.mapEscortRate) mapEscortRate  from  td_billing_args targ join " +
				...
				while (rs.next()) {
					...
					dto.setEscortRate(rs.getDouble("mapEscortRate"));
					dto.setVehicleType(rs.getString("vehicleTypeName"));
				 	 dto.setDistanceRange(rs.getString("mapTripDistance"));
				 	 
					dto.setTotalTrips(rs.getInt("totTrips"));
					dto.setRate(rs.getDouble("mapTripRate"));
					dto.setTrips(getKmBasedMapBillingTripCostReport_tripWise(site, vendorId, dto.getVehicleType(), OtherFunctions.changeDateFromatToIso( dto.getDate()),OtherFunctions.changeDateFromatToIso( dto.getDate()), dto.getMonthName(), dto.getYear(), escort, constraint));
				...
				System.out.println("Error in getKmBasedMapBillingTripCostReport_daily "+ e);
			} finally {
				DbConnect.closeResultSet(rs);
				DbConnect.closeStatement(stmt);
				DbConnect.closeConnection(con);
	...
	public ArrayList<TripDetailsDto> getKmBasedMapBillingTripCostReport_tripWise(String site, String vendorId, String vehicleType, String fromDate, String toDate, String month, String year, String escort, String constraint) {
		...
		//	option=option==null?"":option;
			...
			try {
				...
				String query1 = " select td.trip_code, td.trip_date, td.vehicleTypeName, td.trip_time, td.trip_log, td.security_status, targ.tripId, targ.classicTripBillingId, targ.mapTripRate, tbc.ac_constraint, targ.mapEscortRate,  targ.mapTripDistance from  td_billing_args targ join  " +
						"(select vtp.manualDistance, vtp.escort tEscort, ifnull(vtp.approvalStatus,'Open') approvalStatus, tva.vendorId, td.*, vt.type vehicleTypeName from trip_details td join vehicle_type vt on td.vehicle_type=vt.id join tripvendorassign tva on td.id= tva.tripid  left join vendor_trip_sheet_parent vtp on td.id = vtp.tripId where approvalStatus = 'Approved by Transport Manager' ) td on targ.tripId = td.id" +
						"   join (select tbcc.id, tbcc.ac_constraint, tbcc.trip_rate from km_based_trip_config tbc join km_based_map_trip_config  tbcc on tbcc.km_billing_id=tbc.id where 1=1 "+constraintQuery+"  ) tbc on tbc.id= targ.mapTripBillingId  "+ 
								" where td.siteId="+site+" and td.vendorId="+ vendorId + vehicleQuery + dateQuery+ yearQuery + monthQuery + escortQuery;
				...
				while (rs.next()) {
					...
					dto.setToKm(""+rs.getString("toKm"));
					 dto.setDistanceRange(rs.getString("mapTripDistance")); 
					dto.setTripRate (rs.getDouble("mapTripRate"));
				...
				System.out.println("Error in getKmBasedMapBillingTripCostReport_tripWise "+ e);
			} finally {
				DbConnect.closeResultSet(rs);
				DbConnect.closeStatement(stmt);
				DbConnect.closeConnection(con);
