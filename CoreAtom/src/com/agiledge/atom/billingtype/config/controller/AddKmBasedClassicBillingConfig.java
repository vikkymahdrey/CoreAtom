...
public class AddKmBasedClassicBillingConfig extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddKmBasedClassicBillingConfig() {
        super();
	...
	private void processRequest(HttpServletRequest request,
		...
		try{
			...
			if(service.validateKmBasedClassicBillingTypeConfig(dto) ==false)
			{
				request.getSession().setAttribute("status",
						"<div class=\"failure\" >Validation Failure:!</div>"+service.getMessage());
			}else
			{
			int val=service.addKmBasedClassicConfig(dto);
			if(val>0)
			{
				request.getSession().setAttribute("status",
