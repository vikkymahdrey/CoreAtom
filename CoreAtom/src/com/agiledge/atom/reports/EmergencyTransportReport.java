...
public class EmergencyTransportReport {
	...
	public List<EmergencyDto> getAllEmergencyDetails(String fromDate,
		...
		try {
			...
			String query = "select t.siteId,e.displayname as bookingFor, date_format(t.travelFromDate,'%d/%m/%y') travelFromDate,date_format(t.travelToDate,'%d/%m/%y') travelToDate,t.startTime,e1.displayname as bookingBy, v.regNo as vehicle, t.area,t.place,t.landmark,t.reason,t.booked_date_status,t.booked_time_status from transportation_in_emergency t, employee e ,employee e1, vehicles v where t.bookingFor =e.id and t.bookingBy=e1.id and t.vehicle=v.id and siteId="+siteId+" and travelFromDate between '"+fromDate+"' and '"+toDate+"' and travelToDate between '"+fromDate+"' and '"+toDate+"'";
			...
			while (rs.next()) {
				...
				emerDto.setTravelFromDate(rs.getString("travelFromDate"));
				emerDto.setTravelToDate(rs.getString("travelToDate"));
