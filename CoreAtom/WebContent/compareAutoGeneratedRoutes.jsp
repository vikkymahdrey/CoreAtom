...
ArrayList<ArrayList<RouteDto>> masterlist=null;
	if(routeids!=null && routeids.length>0){
		System.out.println(routeids.length);
...
<body onload="loadScript();">
	...
	<div class="wrapper">
		...
		<div class="main-page-container">
			<div class="container">
				<form action="compareAutoGeneratedRoutes.jsp" method="post"
							...
							<div class="breadcrumb-wrap">
								...
								<%
									if (session.getAttribute("roleId").equals("2")
											|| session.getAttribute("roleId").equals("4")) {
								%>
								...
								<%
									if (session.getAttribute("roleId").equals("2")) {
								%>
								<a href="surveyStats.jsp">Survey Status</a> <a
									href="geoTagReport.jsp">GeoTag Report </a> <a
									href="ShuttleLocations.jsp">Locations </a> <a href="#"
									class="current">Compare Routes </a>
								<%
									}
								%>
							</div>
...
showroutename();
	...
	function showroutename(){
		var cnt=1;
		var msgtable="<table width='80%' ><thead><th> Sl No </th><th> Route Name </th><th> Shift </th><th> Vehicle </th><th> Capacity </th><th> EmployeeCount </th><th> </th></thead>";
		<%for(RouteDto dto : list1){
			String cap="";
		...
		%>
		msgtable+="<tr><td>"+cnt+"</td><td><%=dto.getRouteName()%></td><td><%=dto.getInOut()%></td><td><%=dto.getVehicleType()%></td><td><%=cap%></td><td><%=dto.getVehicleCount()%></td><td><a href='updateRouteUsingEmployeePoints.jsp?routeid=<%=dto.getRouteId()%>'>Edit</a></td></tr>";
		cnt++;
	<%}
	%>
		msgtable+="</table>";
