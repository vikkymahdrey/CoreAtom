...
function  getPath(tripId){
    ...
    var url="GetRoute?tripId="+tripId;
...
function displayPath()
{
    if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
    {
        ...
        var waypts = [];                
        var nodesplit=Math.ceil((noofnodes-1)/8);
        var intnode;
        ...
        {            
            ...
            waypts.push({location:intnodeltln,stopover:false});
        }       
        ...
        directionsService.route(request, function(response, status) {
            if (status == google.maps.DirectionsStatus.OK) {
                ...
                var datasting ='<table border="1" width="460"><thead style="color:#fff;background:#FF6633"><tr><th>SI</th><th>Area</th><th>Place</th><th>Landmark</th></tr></thead>';
                datasting +='<tbody ><tr><td>0</td><td>'+startnode[1]+'</td><td>'+startnode[2]+'</td><td>'+startnode[4]+'</td></tr>';                
                for (var i = 0; i < noofnodes-2; i++) {
                    var intnode=nodes[i+1].split(":");
                    datasting +='<tr><td>'+(i)+'</td><td>'+intnode[1]+'</td><td>'+intnode[2]+'</td><td>'+intnode[4]+'</td></tr>';
                }
                datasting +='<tr><td>'+(i)+'</td><td>'+endnode[1]+'</td><td>'+endnode[2]+'</td><td>'+endnode[4]+'</td></tr></tbody></table>';
...
<body onload="showRoute()">
	...
	<div id="routedet" style="width: 40%; height: 90%; float: right;display: none;"></div>
</body>
