...
.getEmployeeAccurate(session.getAttribute("user")
	...
	<div class="wrapper">
		...
		<div class="main-page-container">
			<div class="container">
				<form action="compareShuttleRouteKeo.jsp" method="post"
...
showroutename();
	...
	function showroutename(){
		var cnt=1;
		var msgtable="<table width='80%' ><thead><th> Sl No </th><th> Route Name </th><th> Shift </th><th> Vehicle </th><th> Capacity </th><th> EmployeeCount </th><th> Route Log </th><th> Route Time </th><th> </th><th> </th></thead>";
		<%for(RouteDto dto : list1){
			String cap="";
		...
		%>
		msgtable+="<tr><td>"+cnt+"</td><td><%=dto.getRouteName()%></td><td><%=dto.getInOut()%></td><td><%=dto.getVehicleType()%></td><td><%=cap%></td><td><%=dto.getVehicleCount()%></td><td><%=dto.getInOut()%></td><td><%=dto.getTime()%></td><td><a href='updateShttleRouteKeo.jsp?routeid=<%=dto.getRouteId()%>'>Edit</a></td><td><input type='button' class='btn btn-blue save-btn' value='Delete' onClick='deleteRoute(<%=dto.getRouteId()%>);'/></td></tr>";
		cnt++;
	<%}%>
		msgtable+="</table>";
	...
	function deleteRoute(routeid){
		if (confirm("Delete selected route?") == true) {
			try {
				var xmlhttp;
				...
				xmlhttp.open("POST", "DeleteAutoRoute?routeid="
						+ routeid , true);
				xmlhttp.send();
			} catch (e) {
