...
<script type="text/javascript">
	...
	function showAll() {
		...
		window.location = "schedule_employee.jsp?site=" + site + "&empFilter="
				+ empFilter + "&Filter=" +
	...
	function next() {
		...
		if (startPos > listSize) {
			...
			/* window.location = "schedule_employee.jsp?site=" + site
					+ "&startPos=" + startPos + "&endPos=" + endPos; */
			window.location = "schedule_employee.jsp?site=" + site
					+ "&empFilter=" + empFilter + "&Filter=" +
...
<body>
	...
	<%
		...
		//SchedulingDao SchedulingDaoObj = new SchedulingDao();
		ArrayList<SchedulingDto> schedulingEmpListinitail;
		...
		site = OtherFunctions.isEmpty(site) == true ? null : site;
		...
		try {
			if ((Filter != null && Filter.length > -1)
					&& OtherFunctions.isEmpty(empFilter) == false) {
				for (String a : Filter) {
					System.out.println("filter data" + a.toString() + ""
							+ empFilter);
				}
			}
		} catch (Exception e) {
		...
		//System.out.println("site : " +site + " employeeFilter: "+ empFilter);
		if (startPos == null && endPos == null) {
			startPos = "0";
			endPos = "" + RECORD_SIZE;
		}

		//schedulingEmpListinitail = SchedulingDaoObj.getSubscribedEmployeeDetailsByManger(actEmpId, empid, site, empFilter);
		schedulingEmpList = SchedulingDaoObj
				.getSubscribedEmployeeDetailsByManger(actEmpId, empid,
						site, empFilter);
		try {

			ArrayList<SchedulingDto> schedulingEmpListinitail2 = new ArrayList<SchedulingDto>();

			schedulingEmpListinitail = SchedulingDaoObj
					.getSubscribedEmployeeDetailsByManger(actEmpId, empid,
							site, empFilter);
			;
			...
			if ((Filter != null && Filter.length > -1)
					&& OtherFunctions.isEmpty(empFilter) == false
					&& (!empFilter.equalsIgnoreCase("--ALL--") && !empFilter
							.equalsIgnoreCase("--Self--"))
					&& schedulingEmpListinitail != null) {
				...
				for (String teamemp : Filter) {
					if (teamemp.equalsIgnoreCase("ALL")) {
						...
						for (SchedulingDto teamempdto : schedulingEmpListinitail) {

							if (teamemp.equals(teamempdto.getEmployeeId())) {

								schedulingEmpListinitail2.add(teamempdto);
								schedulingEmpList.clear();
								schedulingEmpList
										.addAll(schedulingEmpListinitail2);
								flag = "true";
							}
			...
			System.out.println("error in getting schedule details " + e);
		}
		...
		masterLogoutTimeList = logTimeDaoObj.getAllGeneralLogtime("OUT",
				site);
					...
					<table style="width: 40%; border: 0px none;">
						<tr>
							...
							<td>
								...
								%> <select name="empFilter" id="empFilter"
								onchange="return searchForm();">
									<%
							...
							<td id="Filter2"><select name="Filter" id="Filter"
								multiple="true" onchange="return searchForm();">
													...
													for (EmployeeDto edtos2 : subordinateList2) {
														...
														if ((Filter != null && Filter.length > -1)
															...
															for (String empdata : Filter) {
																if (empdata.equalsIgnoreCase(edtos2
																		.getPersonnelNo()))
																	empSelect = " selected ";
															}
									%><option <%=empSelect%> value="<%=edtos2.getPersonnelNo()%>">
										<%=edtos2.getDisplayName()%>&nbsp;&nbsp;--&nbsp;&nbsp;<%=edtos2.getPersonnelNo()%></option>
			...
			<form name="schedule_emp" method="post" action="ScheduleEmployee"
				...
				<table class="dataTable">
					<tr>
						...
						<td></td>
						<td></td>
						<td align="left" width="27%"><input type="checkbox"
							id="checkAllProject" /></td>
					...
					<%
						if (!empFilter.equalsIgnoreCase("--ALL--")
									&& !empFilter.equalsIgnoreCase("--Self--")) {
					...
					<%
						for (SchedulingDto schdulingEmployeeDto : schedulingEmpListIterated) {
								...
								if (schdulingEmployeeDto.getProject() != null
									...
									logoutTimeList = schdulingEmployeeDto
											.getLogoutTimeDto();
					...
					<tr id="<%=serialNo%>">
						...
						<td><%=schdulingEmployeeDto.getDescription()%> <input
							type="hidden" class="project"
 ...
 %></td>
					...
					<tr>
						...
						<td colspan="5" align="center"><input type="submit"
							class="formbutton" name="submit" value="Schedule"
